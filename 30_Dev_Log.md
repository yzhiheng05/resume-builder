...
- 任务：继续让顶部标题更像 Magic Resume 的文件名，而不是身份分类标题
- 操作：将 `identityEditorTitles` 的学生、职场、通用三类顶部标题统一为“新建简历”；保留右侧身份切换器和可访问隐藏身份文本作为身份表达；同步更新渲染测试中的标题断言
- 结果：顶部栏呈现“魔方简历 / 新建简历 / 本地草稿”的文件工作台结构，身份不再重复占用文件名位置；不改身份预设、推荐模块、模板选择、持久化、导入导出或打印逻辑
- 验证：
  - `npm test -- src/test/renderApp.test.tsx src/test/print.test.ts` 通过，62 项测试全部通过
  - `npm test` 通过，95 项测试全部通过
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome 浏览器检查：顶部品牌为“魔方简历”，顶部标题为“新建简历”，当前身份仍在身份切换器中显示；桌面 1440px 与移动 390px 下页面横向溢出均为 0

## 2026-07-07

- 任务：减少产品入口和顶部栏的通用工具文案，让界面更接近 Magic Resume 的品牌化文件工作台
- 操作：将全局品牌常量 `RESUME_TOOL_BRAND` 从“求职简历工具”改为“魔方简历”；将入口页纸面示意工具条文案从“模块化简历”改为“纸面编辑”；同步更新渲染测试
- 结果：顶部栏变为“魔方简历 / 学生简历”一类产品化文件栏，入口页减少“模块化”这类功能说明口吻；不改身份预设、模板、模块数据、持久化、导入导出或打印逻辑
- 验证：
  - `npm test -- src/test/renderApp.test.tsx src/test/print.test.ts` 通过，62 项测试全部通过
  - `npm test` 通过，95 项测试全部通过
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome 浏览器检查：入口页品牌为“魔方简历”，纸面工具条为 `A4纸面编辑`；工作台顶部品牌为“魔方简历”；桌面 1440px 与移动 390px 下页面横向溢出均为 0
...
- 验证：
  ...
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome 浏览器检查：桌面 1440px 与移动 390px 下页面横向溢出均为 0；模块状态点宽高为 6px，字号为 0，文字颜色透明，模块行列宽为 `16px 204px 8px`

## 2026-07-07

- 任务：继续收敛右侧属性面板的按钮和表单控件厚重感，靠近 Magic Resume 的轻量属性栏
- 操作：将模块顶部“复制模块 / 删除模块”按钮改成右对齐透明文本工具按钮；将条目删除按钮进一步缩小为轻量行内动作；将“新增条目”按钮改为白底细边、无内阴影的低对比按钮；统一右侧普通输入框底色为白色；同步更新 CSS 回归测试
- 结果：右侧属性面板不再像传统表单卡片堆叠，模块操作和条目操作更像安静的属性工具行；不改模块复制、删除、新增条目、字段编辑、持久化或 PDF 导出逻辑
- 验证：
  - `npm test -- src/test/renderApp.test.tsx src/test/print.test.ts` 通过，62 项测试全部通过
  - `npm test` 通过，95 项测试全部通过
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome 浏览器检查：桌面 1440px 与移动 390px 下页面横向溢出均为 0；右侧模块动作按钮为透明背景、24px 最小高度、10px 字号；条目删除按钮为透明背景、20px 最小高度；新增条目按钮为白底细边且无阴影

## 2026-07-07

- 任务：继续向 Magic Resume 的干净文档工作台靠拢，减少中心纸张的浮动卡片感和分页调试感
- 操作：将 `.resume-paper` 从无边框多层阴影改为 1px 细边框、1px 圆角和轻阴影；将画布中的“第 1 页结束”文字提示改为仅保留低透明度虚线；把纸张模块选中态从浅黑描边调整为更清楚的低饱和蓝绿描边，并保持无角点、无投影；同步更新 CSS 回归测试
- 结果：中心画布更像简洁正式的文档编辑区，分页提示不再抢占视觉，点击模块后的选中反馈比之前更容易辨认但仍然克制；不改模块选择、拖拽排序、样式控制、持久化、JSON 导入导出或 PDF 打印逻辑
- 验证：
  - `npm test -- src/test/renderApp.test.tsx src/test/print.test.ts` 通过，62 项测试全部通过
  - `npm test` 通过，95 项测试全部通过
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome 浏览器检查：桌面 1440px 与移动 390px 下页面横向溢出均为 0；纸张边框为 `1px solid rgba(18, 18, 18, 0.075)`，圆角为 `1px`，阴影为 `0 2px 8px rgba(18, 18, 18, 0.055)`；分页伪元素内容为空字符串；选中模块 outline 为 `rgba(63, 95, 104, 0.42) solid 1px`

## 2026-07-07

- 任务：继续提升中心简历纸张本身的正式文档感，减少模板装饰
- 操作：将纸张内 `.resume-section` 从带左右内边距和圆角的块状段落改为纯文档流段落；章节标题字重统一降低到 760；校园模板左侧时间线从 1.5mm / 0.16 透明度改为 1mm / 0.1 透明度，并移除每个 section 的圆点伪元素；同步打印样式和 CSS 回归测试
- 结果：简历纸面更接近简洁正式文档，减少“模板控件/时间线装饰”感；不改模块渲染、拖拽选中、模板切换、样式控制、持久化或 PDF 导出逻辑
- 验证：
  - `npm test -- src/test/renderApp.test.tsx src/test/print.test.ts` 通过，62 项测试全部通过
  - `npm test` 通过，95 项测试全部通过
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome 浏览器检查：桌面 1440px 与移动 390px 下页面横向溢出均为 0；校园模板纸张背景为 1mm 细左线；section `border-radius` 为 `0px`，校园 section `::before` 为 `none`

## 2026-07-07

- 任务：继续收敛顶部栏为 Magic Resume 式文件工具栏，减少标题区冗余状态装饰
- 操作：将标题旁的身份文本 `topbar__identity` 改为可访问隐藏，保留右侧身份切换器作为唯一可见身份入口；移除 `topbar__status` 前的小圆点状态灯；同步更新 CSS 回归测试
- 结果：顶部栏更接近“品牌 / 文件名 / 本地草稿 / 操作”的文档工具栏结构，不再在文件名旁重复显示身份胶囊；不改身份切换、状态提示、推荐配置、备份恢复或导出逻辑
- 验证：
  - `npm test -- src/test/renderApp.test.tsx src/test/print.test.ts` 通过，62 项测试全部通过
  - `npm test` 通过，95 项测试全部通过
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome 浏览器检查：桌面 1440px 与移动 390px 下页面横向溢出均为 0；`topbar__identity` 仍保留文本但视觉尺寸为 1px 且 `clip: rect(...)`；`topbar__status::before` 为 `none`

## 2026-07-07

- 任务：继续降低左侧纸张样式面板的控件堆叠感，贴近 Magic Resume 的直接属性控制
- 操作：移除模板区重复的“当前模板”信息行，只保留模板按钮组；将主题色自定义行从白底卡片改为透明分隔行；弱化主题色选中黑框；将数值单位从 `code` 胶囊改为普通辅助文本；同步更新 CSS 回归测试
- 结果：纸张样式面板更像连续属性控件，不再出现重复说明、单位胶囊和强黑框选中态；不改模板切换、主题色、字号、行距、页边距、持久化或导出逻辑
- 验证：
  - `npm test -- src/test/renderApp.test.tsx src/test/print.test.ts` 通过，62 项测试全部通过
  - `npm test` 通过，95 项测试全部通过
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome 浏览器检查：桌面 1440px 下 `.style-board__row` 数量为 0，模板按钮仍为 3 个；主题色自定义行透明无阴影；数值单位透明、无圆角、字体继承；桌面与移动 390px 页面横向溢出均为 0

## 2026-07-07

- 任务：继续简化左侧模块库的添加区域，减少重复图标和胶囊标签带来的模板感
- 操作：将“添加模块”summary 从灰底按钮改为透明分隔行；将模块库条目从“文档图标 + 模块名 + 胶囊动作 + 加号伪元素”改为“模块名 + 轻量动作文字”的两列属性行；移除条目的 `::before` 文档图标、`::after` 加号、禁用态方块标记和动作胶囊；同步更新 CSS 回归测试
- 结果：左侧模块库更像 Magic Resume 式的安静工具列表，不再有重复装饰符号；不改模块添加、单例禁用、展开抽屉、选择或持久化逻辑
- 验证：
  - `npm test -- src/test/renderApp.test.tsx src/test/print.test.ts` 通过，62 项测试全部通过
  - `npm test` 通过，95 项测试全部通过
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome 浏览器检查：桌面 1440px 下 `.module-library__item` 为两列文本，`::before` / `::after` 均为 `none`，动作文字透明无胶囊；桌面与移动 390px 页面横向溢出均为 0

## 2026-07-07

- 任务：继续扁平化右侧属性面板，减少与 Magic Resume 风格不一致的胶囊、灰卡和提示装饰
- 操作：将右侧当前模块 `inspector-context` 从胶囊徽标改为普通灰色文本；照片编辑区从灰底卡片改为属性行；模块提示 note 从带彩色竖条的 callout 改为透明分隔行；个人字段 focus 态移除横向渐变；同步更新 CSS 回归测试
- 结果：右侧面板更像连续的属性列表，减少“组件卡片堆叠”和 AI 模板感；不改照片上传、字段显隐、模块编辑、持久化或导出逻辑
- 验证：
  - `npm test -- src/test/renderApp.test.tsx src/test/print.test.ts` 通过，62 项测试全部通过
  - `npm test` 通过，95 项测试全部通过
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome 浏览器检查：桌面 1440px 与移动 390px 下页面横向溢出均为 0；`inspector-context` 透明、无圆角、无阴影；`photo-editor` 透明、无圆角、无阴影；带提示模块的 `.editor-empty-note::before` 为 `none`
...
- 验证：
  ...
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome/CDP 浏览器检查：桌面 1200px 与移动 390px 下选中模块 outline 为 `rgba(17, 18, 23, 0.2) solid 1px`，`box-shadow` 为 `none`，`::before` / `::after` 均为 `none`，页面无横向溢出

## 2026-07-07

- 任务：继续降低左侧“当前纸面”列表的模板化序号感，贴近 Magic Resume 的工具列表
- 操作：将模块列表左侧的 `01/02/03` 序号替换为 `module-outline__grip` 轻量握柄；弱化当前模块选中态，移除黑色竖条阴影，仅保留低对比边框和背景；同步更新渲染与 CSS 回归测试
- 结果：左侧模块列表更像编辑器工具栏里的可排序模块清单，不再出现明显 AI 模板常见的数字序列装饰；不改模块顺序、选择逻辑、拖拽排序、持久化或导出逻辑
- 验证：
  - `npm test -- src/test/renderApp.test.tsx src/test/print.test.ts` 通过，61 项测试全部通过
  - `npm test` 通过，94 项测试全部通过
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome/CDP 浏览器检查：桌面 1200px 下 9 个模块均渲染 `module-outline__grip`，不再存在 `module-outline__index`，选中项 `box-shadow` 为 `none`，页面无横向溢出；移动 390px 无横向溢出
...
- 任务：让顶部简历名称更接近 Magic Resume 的文件名编辑体验
- 操作：新增 `documentTitle` 状态与默认值，接入本地持久化、V4 迁移、JSON 备份导入导出和顶部栏可编辑输入框
- 结果：顶部不再固定跟随身份标题，用户可以直接编辑并保存简历名称，刷新后保持不丢失
- 验证：
  - `npm test` 通过，97 项测试全部通过
  - `npm run build` 通过
  - Chrome / Playwright 桌面与移动端检查通过：标题可编辑、刷新后保留、无水平溢出
- 后续：
  - 继续按 Magic Resume 的简洁工作台方向打磨左侧栏、属性面板和画布选中态

## 2026-07-07

- 任务：继续向 Magic Resume 风格打磨左侧工具区和画布选中态
- 操作：在左侧栏新增可访问的编辑工具锚点导航；增强当前模块的蓝灰侧边选中条；增强纸面模块选中框、控制点和拖拽手柄可见度；同步补齐打印隐藏规则
- 结果：工作台更接近简洁编辑器工具面板，点击模块后的反馈更明确，但仍保持低对比、纸面优先的视觉风格
- 验证：
  - `npm test` 通过，97 项测试全部通过
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome / Playwright 桌面与移动端检查通过：桌面和移动端无横向溢出；左侧 `编辑工具` 6 个锚点渲染；active 模块左侧选中条保持蓝灰背景；纸面 active outline 为 `rgba(63, 95, 104, 0.62) solid 1px`
- 后续：
  - 继续优化右侧属性面板的密度、标签层级和顶部操作区，进一步降低后台表单感

## 2026-07-07

- 任务：继续降低右侧属性面板的后台表单感
- 操作：将右侧 header 改为“属性”小标签 + 当前模块主标题 + 对象类型徽标；将复制 / 删除模块操作改为更紧凑的工具按钮区；同步更新渲染测试和 CSS 视觉断言
- 结果：右侧属性面板更像对象编辑器，用户选中模块后能直接看到当前编辑对象，整体层级更接近 Magic Resume 的属性栏
- 验证：
  - `npm test` 通过，97 项测试全部通过
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome / Playwright 桌面与移动端检查通过：桌面和移动端无横向溢出；右侧标题显示当前模块名；右上角对象类型标签为 `模块`
- 后续：
  - 继续打磨顶部操作区和右侧字段行细节，减少文字按钮感

## 2026-07-07

- 任务：继续打磨顶部操作区，减少文字按钮堆叠感
- 操作：将文件操作区改为浅色工具轨；为空闲保存状态增加低对比圆点；保留黑色主导出按钮并统一圆角；同步更新顶部栏 CSS 断言
- 结果：顶部栏更像文件编辑器工具条，次要操作更安静，导出操作仍清晰
- 验证：
  - `npm test` 通过，97 项测试全部通过
  - `npm run build` 通过
  - Chrome / Playwright 桌面与移动端检查通过：桌面和移动端无横向溢出；文件操作区背景为 `rgb(248, 248, 247)`；状态圆点渲染；导出按钮保持黑色主按钮
- 后续：
  - 继续减少右侧字段行的视觉噪音，并检查整体移动端首屏观感

## 2026-07-07

- 任务：继续降低右侧属性面板字段行的表格感
- 操作：压缩个人字段和普通文本字段的行高与内边距；降低分隔线透明度；将字段输入默认背景改为透明，仅在 hover / focus 时显示轻边界；减轻重复条目标题颜色和字重；同步更新 CSS 视觉断言
- 结果：右侧属性栏更接近轻量对象属性面板，字段值仍清晰但不再像一格格后台表单
- 验证：
  - `npm test -- src/test/print.test.ts src/test/renderApp.test.tsx` 通过，63 项测试全部通过
  - `npm test` 通过，97 项测试全部通过
  - `npm run build` 通过
  - `git diff --check` 通过
  - Chrome / Playwright 桌面与移动端检查通过：桌面和移动端无横向溢出；个人字段行 `min-height` 为 `36px`；字段输入默认背景为透明；截图人工核对无明显重叠或裁切
- 后续：
  - 继续对照 Magic Resume 打磨整体 UI，优先考虑左侧栏黑白层级、属性面板控件密度和画布区域留白
