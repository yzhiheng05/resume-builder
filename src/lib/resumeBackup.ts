...
export type ImportResumeBackupResult =
  | { ok: true; state: StoredResumeStateV4 }
...
export function createResumeBackupPayload(
  state: StoredResumeStateV4,
  exportedAt = new Date().toISOString()
): ResumeBackupPayloadV4 {
  return {
...
export function serializeResumeBackup(state: StoredResumeStateV4): string {
...
export function parseResumeBackup(raw: string): ImportResumeBackupResult {
  ...
  if (payload.version !== 1 && payload.version !== 2 && payload.version !== 3 && payload.version !== RESUME_BACKUP_VERSION) {
    return { ok: false, error: "文件不是当前应用支持的备份格式。" };
  }
